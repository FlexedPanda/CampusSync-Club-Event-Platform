...
export default function CampusEvents() {
  ...
  const handleProvideFund = async (event, id) => {
    event.preventDefault();
    try {
  ...
  const handleSponsorFund = async (event, id) => {
    event.preventDefault();
    try {
      const res = await axios.post(`/fund/sponsor/${id}`, {amount}, {
        headers: {
          "Content-Type": "application/json",
          token: localStorage.getItem("token"),
          type: localStorage.getItem("type"),
      ...
      if (res.data.offer) {
        toast.success(res.data.message || "Successfully Sponsored Event");
        setEvents(events.filter((event) => event._id !== id));
        setAmount(null);
      } else {
        toast.error(res.data.message || "Failed to Sponsor Event");
      }
      console.log(res.data);
    } catch (error) {
      toast.error("Something Went Wrong");
