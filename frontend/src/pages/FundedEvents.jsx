...
export default function FundedEvents() {
  ...
  const handleWithdraw = async (fundId) => {
    try {
      const res = await axios.post(`/fund/withdraw/${fundId}`, {}, {
        headers: {
          "Content-Type": "application/json",
          token: localStorage.getItem("token"),
        },
      ...
      <div className="h-[90vh] flex items-center justify-center p-6">
        <Card className="transition-transform transform hover:scale-125 shadow-md hover:shadow-lg">
          ...
          <CardContent className="w-full space-y-4 rounded-lg shadow-lg text-center text-gray-600">
            Could Not Find Any Funds Provided
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="h-auto flex items-center justify-center p-6">
      <Card className="w-10/12 rounded-lg shadow-lg bg-slate-100">
        ...
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="text-center text-slate-700">Invoice</TableHead>
                ...
                <TableHead className="text-center text-slate-700">Revenue</TableHead>
                <TableHead className="text-center text-slate-700">Club</TableHead>
                <TableHead className="text-center text-slate-700">Date</TableHead>
                <TableHead className="text-center text-slate-700">Amount</TableHead>
                <TableHead className="text-center text-slate-700">Withdraw</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {funds.map((fund) => (
                <TableRow key={fund._id}>
                  ...
                  <TableCell className="text-center font-semibold text-muted-foreground">
                    <Button
                      ...
                      onClick={() => handleWithdraw(fund._id)}>
                      Withdraw
                    </Button>
                  </TableCell>
                </TableRow>
