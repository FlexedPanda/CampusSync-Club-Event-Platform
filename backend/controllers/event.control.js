...
export const campus = async (req, res) => {
	try {
    const joins = await Join.find({ user: req.user._id });
		if (!joins) {
			return res.status(400).json({
				success: false,
		...
		if (!funds) {
			return res.status(400).json({
				success: false,
				message: "No Fund Provided",
			});
		}

    const joined = joins.map((join) => join.event.toString());
    const events = await Event.find({ _id: { $nin: joined } }).populate("club");
    return res.status(200).json({
      success: true,
...
export const joined = async (req, res) => {
	try {
		const joined = await Join.find({ user: req.user.id })
